...
function buildQuestion(artist,n,globalIndex){
  ...
  if(difficulty==='easy'){
    if(group===0){question=`《${work}》與哪位音樂家關係最密切？`;correct=artist.nameZh;distractors=pickOthers('nameZh',artist);}
    else if(group===1){question=`創作《${work}》的${artist.nameZh}是哪一國音樂家？`;correct=artist.nationality;distractors=pad(pickOthers('nationality',artist),['法國','捷克','美國']);}
    else if(group===2){question=`${artist.nameZh}主要與哪個音樂流派相關？`;correct=artist.artMovement;distractors=pad(pickOthers('artMovement',artist),['文藝復興複音音樂','極簡主義','具象音樂']);}
    else{question=`「${artist.title}」在本遊戲中指哪位音樂家？`;correct=artist.nameZh;distractors=pickOthers('nameZh',artist);}
    ...
    else if(group===1){question=`若研究「${artist.keyTerms[1]}」，最適合先聆聽哪位音樂家？`;correct=artist.nameZh;distractors=pickOthers('nameZh',artist);}
    ...
    else{question=`《${work}》呈現的音樂史線索最接近哪位音樂家？`;correct=artist.nameZh;distractors=pickOthers('nameZh',artist);}
  } else {
    category='analysis';
    if(group===0){question=`將${artist.nameZh}置於音樂史脈絡，哪組「時期／流派」配對正確？`;correct=`${artist.period}／${artist.artMovement}`;distractors=[...new Set(ARTISTS.filter(a=>a.id!==artist.id).map(a=>`${a.period}／${a.artMovement}`))].filter(x=>x!==correct).slice(0,3);}
    else if(group===1){question=`下列哪部作品最適合用來討論${artist.nameZh}的「${artist.keyTerms[0]}」？`;correct=work;distractors=[...new Set(ARTISTS.filter(a=>a.id!==artist.id).map(a=>a.representativeWorks[0]))].filter(title=>title!==work).slice((n+3)%14,(n+3)%14+3);}
    else if(group===2){question=`比較音樂家年代，哪一位的出生年是 ${artist.birthYear}？`;correct=artist.nameZh;distractors=pickOthers('nameZh',artist);}
    else{question=`哪位音樂家同時符合「${artist.artMovement}」與「${artist.keyTerms[2]}」？`;correct=artist.nameZh;distractors=pickOthers('nameZh',artist);}
  }
