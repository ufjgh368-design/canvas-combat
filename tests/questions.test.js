...
test('每位音樂家有 10 題且三級各至少 60 題',()=>{const r=validateQuestions();assert.equal(QUESTIONS.length,340);assert.ok(Object.values(r.counts).every(count=>count>=60));assert.equal(r.valid,true,r.errors.join('\n'));});
